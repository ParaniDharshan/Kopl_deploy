...
export default function Testimonal() {
  ...
  const getCenteredOffset = useCallback(
    (index) => {
      ...
      const centered =
        viewportWidth / 2 - CARD_GAP - index * CARD_STEP - CARD_WIDTH / 2;
  ...
  const maskL = isDark
    ? "linear-gradient(90deg,rgba(3,12,24,1),transparent)"
    : "linear-gradient(90deg,rgba(240,248,255,1),transparent)";
  const maskR = isDark
    ? "linear-gradient(270deg,rgba(3,12,24,1),transparent)"
    : "linear-gradient(270deg,rgba(240,248,255,1),transparent)";
    ...
    <Box sx={{ py: { xs: 8, md: 10 }, background: bg, overflow: "hidden" }}>
        ...
        >
          ...
          >
            {items.map((t, idx) => {
              ...
              const rating =
                typeof t.rating === "number" ? t.rating : DEFAULT_RATING;
              const initials =
                t.avatar ||
                (t.name
                  ? t.name
                  ...
                  >
                    {Array.from({ length: 5 }).map((_, si) =>
                      si < rating ? (
                        ...
                        <StarBorderIcon
                          key={si}
                          sx={{ fontSize: 13, color: PRIMARY, opacity: 0.28 }}
                        />
                      ),
                    )}
                    <Typography
              ...
              background:
                i === centeredIdx
                  ...
                  : isDark
                    ...
                    : "rgba(8,45,74,0.14)",
              transition:
                "width 0.35s cubic-bezier(.22,1,.36,1), background 0.35s ease, transform 0.2s ease",
              cursor: "pointer",
              appearance: "none",
              outline: "none",
              display: "block",
      ...
      <Box sx={{ py: { xs: 10, md: 12.5 }, textAlign: "center" }}>
        <Container maxWidth="sm">
          <Typography
            variant="h4"
            sx={{ mb: 2, fontWeight: 700, color: textPri }}
          >
